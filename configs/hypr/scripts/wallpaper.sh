...
theme_wallpaper_dir="$HOME/.config/homenix/current/theme/backgrounds"
scripts_dir="$HOME/.config/hypr/scripts"
wallpaper_current="$HOME/.config/hypr/wallpaper_effects/.wallpaper_current"
rofi_theme="$HOME/.config/rofi/config-wallpaper.rasi"

# Directory for swaync
iDIR="$HOME/.config/homenix/current/swaync/images"
iDIRi="$HOME/.config/homenix/current/swaync/icons"
...
mapfile -d '' PICS < <(find -L "${wallDIR}" "${theme_wallpaper_dir}" -type f \( \
...
# Rofi command
rofi_command="rofi -i -show -dmenu -config $rofi_theme -theme-str $rofi_override -window-title Background"
...
#   #     --button="no:1"; then
#   #
#   #     exec "$scripts_dir/sddm_wallpaper.sh" --normal
#   #
#   #   fi
#   # fi
# }
...
apply_image_wallpaper() {
  ...
  "$scripts_dir/wallust.sh" "$image_path"
  sleep 2
...
main() {
  ...
  selected_file=$(find "$wallDIR" "${theme_wallpaper_dir}" -iname "$choice_basename.*" -print -quit)
  ...
  ln -sf $selected_file $HOME/.config/homenix/current/background
