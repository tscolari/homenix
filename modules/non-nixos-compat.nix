...
in
{
  config = mkIf (config.programs.homenix.enable && isNonNixOS) {
    ...
    xdg.configFile."uwsm/env-nonixos".text = mkIf config.programs.homenix.hyprland.useUWSM ''
      ...
      export GI_TYPELIB_PATH="${config.home.profileDirectory}/lib/girepository-1.0:$HOME/.nix-profile/lib/girepository-1.0:/usr/lib/x86_64-linux-gnu/girepository-1.0:/usr/lib/girepository-1.0"
    '';
  };
}
